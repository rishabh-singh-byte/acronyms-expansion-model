...
| # | File | Records (Nov 2025) | Structure | Primary Use | Origin |
|---|------|-------------------|-----------|-------------|--------|
| 1 | `app/data/acronyms_list_cleaned.json` | 8,535 acronyms → 59,720 expansions | `dict[str, list[str]]` | Lookup table for acronym extraction & prompting | Export from `Notebooks/acronyms_extraction.ipynb` after cleaning raw acronym corpus |
| 2 | `app/data/golden_data_20k.json` | 20,000 rows | `list[{"Query","Candidate_Acronyms","Best_Output"}]` | Canonical “golden” benchmark used for random sampling, demos, and QA | Curated ground-truth set assembled offline and checked in for reproducibility |
| 3 | `app/data/sampled_20000_queries.json` | 20,000 rows | `list[{"query","candidate_acronyms","output"}]` | Normalized query/candidate pairs fed into inference jobs | Generated by `Notebooks/acronyms_expansion.ipynb` when sampling from the 125k-query pool |
| 4 | `app/evaluation_v1/results/converted_output_llama.json` | 6,284 rows | `list[{"index","Query","model_1","model_2"}]` | Normalized TinyLlama vs GPT responses ready for judging | Produced inside `evaluation_v1/results/evaluation_gpt.ipynb` to split concatenated keys before scoring |
| 5 | `app/evaluation_v1/results/mismatched_outputs_qwen_base.json` | 10,921 rows | `list[{"index","Query","model_1","model_2"}]` | Raw disagreements between ground-truth and Qwen-base outputs | Exported by `app1/test.ipynb` after diffing `base_results_20000.xlsx` against expected labels |
| 6 | `app/evaluation_v1/results/mismatched_outputs_qwen(ft).json` | 4,488 rows | Same as #5 | Raw disagreements for Qwen LoRA fine-tune | Exported by `app1/test.ipynb` (Qwen-QLoRA mismatch cell) from `best_model_outputs_1a.xlsx` |
| 7 | `app/evaluation_v1/results/mismatched_outputs_llama_2nd.json` | 5,621 rows | `list[{"index","Query","model_1_gpt","model_2_llama"}]` | Raw disagreements for TinyLlama LoRA vs GPT | Exported by `app1/test.ipynb` after comparing `llama1B_results_20_lora.xlsx` to expected outputs |
| 8 | `app/evaluation_v1/results/mismatched_evaluation_results_gpt_base.json` | 10,921 rows | `list[{"query","model_1_qwen_base","model_2_gpt","candidate_acronyms","better_model","justification"}]` | GPT-4o-mini verdicts judging Qwen-base vs GPT mismatches (Model 1=Qwen) | `gpt_qwen_evaluation.py` with `mismatched_outputs_qwen_base.json` as input |
| 9 | `app/evaluation_v1/results/mismatched_evaluation_results_gpt_base_2.json` | 10,921 rows | Same as #8 but keys flipped (`model_1_gpt`) | Second pass with model positions swapped to remove ordering bias | Same `gpt_qwen_evaluation.py`, rerun with swapped role assignment |
| 10 | `app/evaluation_v1/results/mismatched_evaluation_results_gpt_qwen(ft)1.json` | 4,488 rows | GPT verdicts for GPT (Model 1) vs Qwen-QLoRA (Model 2) | Judge fine-tuned model against GPT | `gpt_qwen_evaluation.py` pointed at `mismatched_outputs_qwen(ft).json` |
| 11 | `app/evaluation_v1/results/mismatched_evaluation_results_gpt_qwen(ft)2.json` | 4,488 rows | Same as #10 but Model roles swapped | Bias-checked rerun for fine-tuned vs GPT | Same script with inverted role mapping |

Record counts were recomputed with `python` on 2025‑11‑19 to confirm the files’ health.
...
### `converted_output_llama.json`
- **Why it exists:** TinyLlama sometimes wrote answers such as `"ai": ["artificial intelligence", "quantexa: ..."]`. `evaluation_v1/results/evaluation_gpt.ipynb` rewrites those blobs so each acronym is its own key, preventing the evaluator from penalizing formatting artifacts.
- **Shape:** 6,284 entries where at least one query needed restructuring; keys are `model_1` (GPT) and `model_2` (cleaned TinyLlama dicts).
- **Usage:** Feed this file (instead of the raw mismatches) into `gpt_llama_evaluation.py` when judging TinyLlama vs GPT.
...
- Although the judged TinyLlama outputs (`mismatched_evaluation_results_gpt_llama_nano*.json`) are stored at the repo root, they come from the same judge script (`gpt_llama_evaluation.py`) using `converted_output_llama.json` as input and likewise run twice with swapped ordering.
...
3. **Normalize as needed**—rerun `evaluation_v1/results/evaluation_gpt.ipynb` if TinyLlama formats changed.
4. **Judge with GPT** by running `python app/evaluation_v1/gpt_qwen_evaluation.py` or `python app/evaluation_v1/gpt_llama_evaluation.py`. 
